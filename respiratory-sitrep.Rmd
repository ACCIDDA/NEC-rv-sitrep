...
#### **COVID-19**
  - The **National** epidemic trend is **not changing**.
    - The weekly percentage of ED visits diagnosed with COVID-19 is **very low**.
    - Probability epidemic is growing: **56.23%** (not changing).
  - **Utah:** The epidemic trend is **not estimated**.
    - The weekly percentage of ED visits diagnosed with COVID-19 is **very low**. 
    - Probability epidemic is growing: **NA%** (not estimated).
  - **Arizona:** The epidemic trend is **not changing**.
    - The weekly percentage of ED visits diagnosed with COVID-19 is **very low**.
    - Probability epidemic is growing: **57.03%** (not changing).
  - **New Mexico:** The epidemic trend is **growing**.
    - The weekly percentage of ED visits diagnosed with COVID-19 is **very low**. 
    - Probability epidemic is growing: **95.93%** (growing).

#### **Influenza**
  - The **National** epidemic trend is **growing**.
    - The weekly percentage of ED visits diagnosed with Influenza is **low**.
    - Probability epidemic is growing: **99.80%** (growing).
  - **Utah:** The epidemic trend is **not estimated**.
    - The weekly percentage of ED visits diagnosed with Influenza is **low**.
    - Probability epidemic is growing: **NA%** (not estimated).
  - **Arizona:** The epidemic trend is **growing**.
    - The weekly percentage of ED visits diagnosed with Influenza is **very low**. 
    - Probability epidemic is growing: **99.80%** (growing).
  - **New Mexico:** The epidemic trend is **growing**.
    - The weekly percentage of ED visits diagnosed with Influenza is **very low**. 
    - Probability epidemic is growing: **91.45%** (growing).

#### **RSV**
  - No state-level or national estimates are estimated for RSV this week.
    - Probability epidemic is growing: **NA%** (not estimated).
...
#temp
forecast_date <- as.Date("2025-09-27")
...
# Combine and attach location_name / abbreviation using your existing helper
hub_forecasts <- dplyr::bind_rows(
  flu_forecasts,
  covid_forecasts,
  rsv_forecasts
)

hub_forecasts <- ensure_locations(
  hub_forecasts,
  geo_ids     = geo_ids,
  state_names = state_names,
  loc_data    = ld
...
# 
# flu_ens <- c("FluSight-ensemble")
# covid_ens <- "CovidHub-ensemble"
# rsv_ens <- "RSVHub-ensemble"
# 
# 
# # copy Forecast Hub forecasts to model-output folder
# copy_fch_outputs(flu_repo_dir,
#                  forecast_date,
#                  models_to_copy = flu_ens)
# ```
...
# Prepare recent/current slices (group-wise) exactly as before
nhsn_recent <- nhsn %>%
...
# Plot NHSN with last-year overlay (previous season dashed, alpha = 0.5)
# Legend: color = Disease, linetype = Season with the requested labels
ggplot(nhsn_plot_data, aes(x = target_end_date, y = observation, color = disease_label, linetype = season, alpha = season)) +
  geom_line(size = 1) +
  geom_vline(xintercept = nhsn$issue_date, linetype = "dotdash") +
  facet_wrap(~ toupper(geo_value), scales = "free_y", ncol = 3) +
  scale_color_manual(values = okabe_ito, name = "Disease") +
  scale_linetype_manual(
    name = "Season",
    values = c("Current" = "solid", "Previous season" = "dashed"),
    labels = c("Current" = "Current season (2025-'26)", "Previous season" = "Previous season (2024-'25)")
  ) +
...
# NSSP: do the same extension for week_end / ed_pct
nssp_recent <- nssp %>%
...
ggplot(nssp_plot_data, aes(x = week_end, y = ed_pct, color = disease_label, linetype = season, alpha = season)) +
  geom_line(size = 1) +
  geom_vline(xintercept = nssp$issue, linetype = "dotdash") +
...
kpi <- full_join(kpi_hosp, kpi_ed, by = c("State","Virus")) %>%
  arrange(State, Virus)
...
#### This report describes recent state-level trends in respiratory virus activity in Arizona, New Mexico, and Utah. It presents data on three respiratory viral diseases: COVID-19, influenza (flu), and respiratory syncytial virus (RSV). Trends in emergency department visits are based on [National Syndromic Surveillance Program (NSSP)](https://www.cdc.gov/nssp/php/about/index.html) data and provide estimates of how much symptomatic illness each virus is causing. Hospitalization trends are drawn from [National Healthcare Safety Network (NHSN)]( https://www.cdc.gov/nhsn/about-nhsn/index.html) data and estimate the burden of severe disease and impact on the healthcare system caused by each virus.