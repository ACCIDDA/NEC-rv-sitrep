...
summary_bullets <- c(
    "The current COVID epidemic trend (based on data reported through 12/09/2025) is **declining** for Arizona, **likely growing** for Colorado, **not changing** for New Mexico, and is not currently estimated for Utah. The weekly percentage of ED visits diagnosed with COVID was **very low** for Arizona and Utah, and **low** for Colorado and New Mexico.",
  
  "The current flu epidemic trend (based on data reported through 12/09/2025) is **likely growing** for Arizona, **growing** for Colorado and New Mexico, and is not currently estimated for Utah. The weekly percentage of ED visits diagnosed with flu was **low** for Arizona, New Mexico and Utah, and **moderate** for Colorado.",
  
    "The current RSV epidemic trend  (based on data reported through 12/09/2025) is **growing** for Arizona, Colorado and New Mexico, but is not currently estimated for New Mexico or Utah. The weekly percentage of ED visits diagnosed with RSV was **very low** for Arizona, New Mexico and Utah, and **low** for Colorado."
)
...
ggplot(nhsn_plot_data, aes(x = target_end_date, y = observation, color = disease_label, linetype = season, alpha = season)) +
  geom_line(size = 1) +
  # Forecast interval ribbon (optional)
  geom_ribbon(
    data = hub_fc_pi,
    aes(x = target_end_date, ymin = fc_lo, ymax = fc_hi, fill = disease_label),
    inherit.aes = FALSE,
    alpha = 0.15
  ) +
  ...
  geom_line(
    data = hub_fc_med,
    aes(x = target_end_date, y = fc_med, color = disease_label),
    inherit.aes = FALSE,
    linewidth = 1,
    linetype = "longdash"
  ) +
