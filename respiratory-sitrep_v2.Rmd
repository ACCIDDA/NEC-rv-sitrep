...
pull_hub_forecasts <- function(repo_dir,
  ...
  purrr::map_dfr(model_dirs, function(m) {
    # Use the forecast_date passed into the function (not a global hub_date)
    file_path <- file.path(
      model_output_dir,
      m,
      sprintf("%s-%s.csv", as.character(forecast_date), m)
    )

    if (!is.character(file_path) || length(file_path) != 1) {
      message("Skipping model ", m, " because file_path is not a single string: ", str(file_path))
      return(tibble::tibble())
    }

    if (!file.exists(file_path)) {
      # no file for this model/date -> return empty tibble to keep map_dfr shape consistent
      return(tibble::tibble())
    }

    # Read defensively: ensure 'location' is always read/coerced to character to avoid bind_rows type conflicts
    df <- tryCatch(
      {
        readr::read_csv(
          file_path,
      ...
      error = function(e) {
        message("Failed to read ", file_path, ": ", conditionMessage(e))
        return(tibble::tibble())
      }
    )
    ...
    # Defensive normalization in case the reader did not produce character
    if ("location" %in% names(df)) {
      df <- dplyr::mutate(df, location = as.character(.data$location))
    } else {
