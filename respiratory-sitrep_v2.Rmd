...
hub_date <- as.character(floor_date(today, unit = "week", week_start = 7) - 1 + 7)
...
summary_bullets <- c(
    "The current COVID epidemic trend (based on data reported through 1/6/2026) **declining** for Arizona and New Mexico, **not changing** for Colorado, and is not currently estimated for Utah. The weekly percentage of ED visits diagnosed with COVID was **low** for Colorado and New Mexico and **very low** for Arizona and Utah.",
  
  "The current flu epidemic trend (based on data reported through 1/6/2026) is **declining** for Arizona, Colorado, and New Mexico, and is not currently estimated for Utah. The weekly percentage of ED visits diagnosed with flu was **very high** for Colorado and Utah, **high** for New Mexico, and **moderate** for Arizona.",
  
    "The current RSV epidemic trend  (based on data reported through 1/6/2026) is **growing** for Colorado and New Mexico, **likely growing** for Arizona, and is not currently estimated for Utah. The weekly percentage of ED visits diagnosed with RSV was **low** for Arizona, Colorado, New Mexico, and Utah."
)
...
# Vaccination Coverage by Disease
...
# ---- RSV ----
rsv_season_col <- find_season_col(rsv_vax, c("season", "respiratory_season", "rsv_season"))
rsv_vax <- rsv_vax %>%
  mutate(week_ending = lubridate::ymd(week_ending),
...
# COVID plot
sc2_g1 <- ggplot() +
  # previous-season overlay (dotted, alpha)
  geom_line(
    data = prev_vax_sc2,
    aes(x = week_ending, y = estimate, color = color_group, group = geographic_name),
    linewidth = 0.9, linetype = "dotted", alpha = 0.5
  ) +
  # current season
  geom_line(
    data = current_vax_sc2,
    aes(x = week_ending, y = estimate, color = color_group, linetype = line_type, group = geographic_name),
    linewidth = 1
  ) +
  geom_point(data = current_vax_sc2, aes(x = week_ending, y = estimate, color = color_group, group = geographic_name), size = 2) +
  scale_x_date(date_labels = "%Y-%m-%d") +
  scale_y_continuous(labels = label_number(scale = 1, suffix = "%")) +
  scale_color_manual(values = vax_colors, breaks = c("Arizona", "Colorado", "New Mexico", "Utah", "National"), name = "Geography") +
  scale_linetype_manual(values = c("State" = "solid", "National" = "longdash"), guide = "none") +
  labs(title = "Adult (18+) COVID Vaccination Coverage — National + AZ/CO/NM/UT",
       x = "Week ending",
...
# Influenza plot (note: flu y-column is named 'estimates' rather than 'estimate')
flu_g1 <- ggplot() +
  geom_line(
    data = prev_vax_flu,
    aes(x = week_ending, y = estimates, color = color_group, group = geographic_name),
    linewidth = 0.9, linetype = "dashed", alpha = 0.5
  ) +
  geom_line(
    data = current_vax_flu,
    aes(x = week_ending, y = estimates, color = color_group, linetype = line_type, group = geographic_name),
    linewidth = 1
  ) +
  geom_point(data = current_vax_flu, aes(x = week_ending, y = estimates, color = color_group, group = geographic_name), size = 2) +
  scale_x_date(date_labels = "%Y-%m-%d") +
  scale_y_continuous(labels = label_number(scale = 1, suffix = "%")) +
  scale_color_manual(values = vax_colors, breaks = c("Arizona", "Colorado", "New Mexico", "Utah", "National"), name = "Geography") +
  scale_linetype_manual(values = c("State" = "solid", "National" = "longdash"), guide = "none") +
  labs(title = "Adult (18+) Flu Vaccination Coverage — National + AZ/CO/NM/UT",
       x = "Week ending",
       y = "Coverage (% Up-to-date)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
rsv_g1 <- ggplot() +
  geom_line(
    data = current_vax_rsv,
    aes(x = week_ending, y = estimate, color = color_group, linetype = line_type, group = geography_name),
    linewidth = 1
  ) +
  geom_point(data = current_vax_rsv, aes(x = week_ending, y = estimate, color = color_group, group = geography_name), size = 2) +
  scale_x_date(date_labels = "%Y-%m-%d") +
  scale_y_continuous(labels = label_number(scale = 1, suffix = "%")) +
  scale_color_manual(values = vax_colors, breaks = c("Arizona", "Colorado", "New Mexico", "Utah", "National"), name = "Geography") +
  scale_linetype_manual(values = c("State" = "solid", "National" = "longdash"), guide = "none") +
  labs(title = "Adults 50+ RSV Vaccination Coverage — National + AZ/CO/NM/UT",
       x = "Week ending",
       y = "Coverage (% Up-to-date)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
<div class="summary-box">
  <ul style="list-style-type: none; padding-left: 0;">
    <li>
        <p>
      These figures show  national and state-level trends in vaccination coverage against COVID-19, flu, and RSV based on National Immunization Survey (NIS) Fall Respiratory Virus Module data. Data are shown for the 2025-2026 respiratory virus season, beginning October 2025 and represent percentage of the (age-specific) population with 'up-to-date' vaccination against COVID-19, flu, or RSV. <br>
      <b>
      </b>
 Coverage for COVID-19 and flu is shown for all individuals over 18, while RSV vaccination is shown for adults 50 years and older. State-specific estimates for pediatric RSV vaccination coverage were not available at this time.
      </p>
      <ul>
      </ul>
    </li>
