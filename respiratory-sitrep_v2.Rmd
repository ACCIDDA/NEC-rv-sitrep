...
/* enlarge NEC (first) */
.rpt-top-logos img:nth-child(1) { max-height: 128px !important; }

/* shrink ACCIDDA (third) */
.rpt-top-logos img:nth-child(3) { max-height: 72px !important; }
...
logo_paths <- c(
    'C:/Users/milla/OneDrive/Desktop/logos/NEC-logo-1000.jpg',
    'C:/Users/milla/OneDrive/Desktop/logos/BSPH.logo.rgb_horizontal.blue.png',
    'C:/Users/milla/OneDrive/Desktop/logos/ACCIDDALogoSmall.png'
)
...
# Paired layout: for each disease emit a single row with a left summary box
# and the choropleth on the right. Uses the CDC "Epidemic Trends and Rt" dataset.

# --- User config ---
states_to_show <- c("Arizona", "New Mexico", "Utah")
...
summary_bullets <- c(
  "The current COVID epidemic trend is not estimated for Utah, is not changing for Arizona, and is growing for New Mexico. The weekly percentage of ED visits diagnosed with COVID is very low for Utah, Arizona, and New Mexico. Hospital admissions for COVID are currently decreasing in Arizona, increasing in New Mexico, and stable in Utah.",
  "The current flu epidemic trend is not estimated for Utah, but is growing for Arizona and New Mexico. The weekly percentage of ED visits diagnosed with flu is not estimated for Utah but is very low for Arizona and New Mexico. Hospital admissions for flu are currently stable in Utah and increasing in New Mexico and in Utah.",
  "No state-level estimates are currently available for RSV. RSV burden remains low across Arizona, New Mexico, and Utah."
)
# --------------------
...
.summary-item {
  ...
  min-height: 360px;            /* approximate height to match map */
  display: flex;
...
/* right plot wrapper adds padding so text and plot don't touch */
.plot-wrap {
  padding-left: 12px;
  padding-bottom: 8px;
}
...
p <- ggplot() +
  ...
  geom_text(data = centroids %>% filter(region %in% regions_to_keep),
            aes(x = long, y = lat, label = toupper(substr(rt_state,1,2))),
